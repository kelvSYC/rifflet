...
/**
 * Translates a domain object of type [T] into the ordered children of a `CAT ` chunk.
 *
 * Unlike [ChunkEncoder], which writes raw bytes into a buffer, a `CatChunkEncoder` produces a structured
 * ordered list of typed child values. The encoder infrastructure ([IffEncoderCore]) is responsible for
 * serializing each child using its registered encoder and assembling the `CAT ` chunk body, including
