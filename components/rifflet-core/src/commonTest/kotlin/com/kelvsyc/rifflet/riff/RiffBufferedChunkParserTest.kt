...
import com.kelvsyc.rifflet.internal.riff.RiffBufferedChunkParser
import io.kotest.core.spec.style.FunSpec
import io.kotest.matchers.shouldBe
import okio.Buffer

class RiffBufferedChunkParserTest : FunSpec({
    ...
    context("even-sized chunk") {
        test("type, declared size, and data are all read correctly") {
            ...
            val chunk = RiffBufferedChunkParser.parse(source)
    ...
    context("size field endianness") {
        test("size is read as little-endian") {
            // Size = 0x00000100 = 256 in little-endian is bytes [00 01 00 00]
            val source = Buffer().apply {
                writeString("TEST", Charsets.ISO_8859_1)
                writeByte(0x00); writeByte(0x01); writeByte(0x00); writeByte(0x00) // LE 256
                write(ByteArray(256))
            }
            val chunk = RiffBufferedChunkParser.parse(source)
