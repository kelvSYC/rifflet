...
v1 ships one rule (`TerrEntry.pollutionEffect` cross-reference bounds checking); the `ValidationRule` contract is general enough to grow into era-dependent rules, bit/enum validity, and cross-entry or whole-file constraints (section cardinality, required section sets, and so on) without changing shape.
