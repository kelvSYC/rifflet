 ...
 * files (confirmed against an actual Conquests scenario) show 7 flavors per group, each with a
 * 7-element [relations] vector forming a personality/relationship affinity matrix (self-affinity
 * highest, others lower) — but the file format declares both the flavor count (see
 * [FlavGroupEntry]) and this per-flavor relation count dynamically, so this type models them as
 * such rather than assuming fixed sizes.
 *
