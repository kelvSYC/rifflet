 ...
 * `QueryCiv3`'s own authors describe this as their least-complete, most tangled struct.
 *
 ...
 *   either primary source.
 * @param unknown2 1 byte with zero documented behavior from either primary source;
 ...
 *   preserved raw, not decomposed. Opaque — neither primary source names any bits for this field
 *   (see [TerrEntry.terrainFlags]'s KDoc for which sources were checked).
 * @param unknown3 1 byte with zero documented behavior from either primary source;
 ...
 *   genuinely differ.
 * @param unknown4 2 bytes with zero documented behavior from either primary source;
 *   present only in [Civ3FormatEra.CONQUESTS] files, read defensively; preserved raw, not
 *   validated.
 ...
 * @param unknown5 2 bytes with zero documented behavior from either primary source;
 *   present only in [Civ3FormatEra.CONQUESTS] files, read defensively; preserved raw, not
 *   validated.
 * @param unknown6 4 bytes, undocumented by any primary source and not part of `QueryCiv3`'s
 *   struct at all. Present only in a small minority of [Civ3FormatEra.CONQUESTS] (`major=12`)
